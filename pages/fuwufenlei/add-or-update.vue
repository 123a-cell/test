	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`fuwufenlei`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				//跨表计算判断
				var obj;
				if((!this.ruleForm.fuwufenlei)){
					this.$utils.msg(`服务分类不能为空`);
					return
				}
				//更新跨表属性
				var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`fuwufenlei`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`fuwufenlei`, this.ruleForm);
						}else{
							oet = await this.$api.add(`fuwufenlei`, this.ruleForm);
						}
