	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		<view class="content">
			<view :style='{"minHeight":"100%","padding":"0 20rpx 20rpx 20rpx","flexWrap":"wrap","background":"#F4F4F4","display":"flex","width":"100%","position":"relative","height":"auto"}'>
						...
						<block v-for="(product,index) in list" :key="index">
							<view v-if="index%2==0" class="list-item" :style='{"boxShadow":"none","padding":"12rpx","margin":"0 0 20rpx","flexWrap":"wrap","borderWidth":"2rpx","borderImage":"linear-gradient(180deg, rgba(145.00000655651093, 219.0000021457672, 232.00000137090683, 1), rgba(245.00000059604645, 195.0000035762787, 229.00000154972076, 1)) 1 1","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
								...
								<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
									<view :style='{"padding":"4rpx 8rpx","background":"linear-gradient( 90deg, #F5C3E5 0%, #91DBE8 100%)","display":"flex"}' v-if="(userid && isAuth('smsregistercode','修改')) || (!userid && isAuthFront('smsregistercode','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
									...
									<view :style='{"padding":"4rpx 8rpx","background":"#91DBE8","display":"flex"}' v-if="(userid && isAuth('smsregistercode','删除')) || (!userid && isAuthFront('smsregistercode','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
								...
								<!-- #ifndef MP-WEIXIN -->
								<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
									<view :style='{"padding":"4rpx 8rpx","background":"linear-gradient( 90deg, #F5C3E5 0%, #91DBE8 100%)","display":"flex"}' v-if="(userid && isAuth('smsregistercode','修改')) || (!userid && isAuthFront('smsregistercode','修改'))" @tap.stop.proevent="onUpdateTap(product)">
										<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-edit"></text>
										<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>修改</text>
									</view>
									<view :style='{"padding":"4rpx 8rpx","background":"#91DBE8","display":"flex"}' v-if="(userid && isAuth('smsregistercode','删除')) || (!userid && isAuthFront('smsregistercode','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
										<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}' class="cuIcon-delete"></text>
										<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#fff","display":"inline-block"}'>删除</text>
									</view>
								</view>
				...
				<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"20rpx ","color":"rgb(255, 255, 255)","outline":"none","borderRadius":"20rpx","background":"linear-gradient( 104deg, #91DBE8 0%, #F5C3E5 100%)","width":"120rpx","lineHeight":"70rpx","fontSize":"28rpx","position":"relative","height":"70rpx","zIndex":"1","order":"3"}' v-if="userid && isAuth('smsregistercode','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"20rpx ","color":"rgb(255, 255, 255)","outline":"none","borderRadius":"20rpx","background":"linear-gradient( 104deg, #91DBE8 0%, #F5C3E5 100%)","width":"120rpx","lineHeight":"70rpx","fontSize":"28rpx","position":"relative","height":"70rpx","zIndex":"1","order":"3"}' v-if="!userid && isAuthFront('smsregistercode','新增')" class="add-btn" @click="onAddTap()">新增</button>
			</view>
		</view>
	</mescroll-uni>
</template>
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('smsregistercode', JSON.stringify([id]));
